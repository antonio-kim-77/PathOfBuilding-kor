...
local influenceDropdownNames = { "없음" }
...
function TradeQueryGeneratorClass:StartQuery(slot, options)
	...
	local controls = { }
	controls.progressText = new("LabelControl", {"TOP",nil,"TOP"}, {0, 30, 0, 16}, string.format("모드 가중치 계산 중..."))
	self.calcContext.popup = main:OpenPopup(280, 65, "잠시 기다려 주세요", controls)
end
...
function TradeQueryGeneratorClass:FinishQuery()
	...
	if #queryTable.query.stats[1].filters == 0 then
		-- No mods to filter
		errMsg = "검색을 생성할 수 없습니다. 검색할 모드를 찾지 못했습니다"
	end
...
function TradeQueryGeneratorClass:RequestQuery(slot, context, statWeights, callback)
	...
	controls.includeCorrupted = new("CheckBoxControl", {"TOP",nil,"TOP"}, {-40, 30, 18}, "타락 모드:", function(state) end)
	...
	controls.includeMirrored = new("CheckBoxControl", {"TOPRIGHT",lastItemAnchor,"BOTTOMRIGHT"}, {0, 5, 18}, "거울 아이템:", function(state) end)
	...
	if not isJewelSlot and not isAbyssalJewelSlot and includeScourge then
		controls.includeScourge = new("CheckBoxControl", {"TOPRIGHT",lastItemAnchor,"BOTTOMRIGHT"}, {0, 5, 18}, "스컬지 모드:", function(state) end)
	...
	if isAmuletSlot then
		controls.includeTalisman = new("CheckBoxControl", {"TOPRIGHT",lastItemAnchor,"BOTTOMRIGHT"}, {0, 5, 18}, "탈리스만 모드:", function(state) end)
	...
	if isEldritchModSlot then
		controls.includeEldritch = new("CheckBoxControl", {"TOPRIGHT",lastItemAnchor,"BOTTOMRIGHT"}, {0, 5, 18}, "엘드리치 모드:", function(state) end)
	...
	if isJewelSlot then
		...
		controls.jewelTypeLabel = new("LabelControl", {"RIGHT",controls.jewelType,"LEFT"}, {-5, 0, 0, 16}, "주얼 유형:")
	...
	elseif slot and not isAbyssalJewelSlot then
		...
		controls.influence1Label = new("LabelControl", {"RIGHT",controls.influence1,"LEFT"}, {-5, 0, 0, 16}, "영향력 1:")
		...
		controls.influence2Label = new("LabelControl", {"RIGHT",controls.influence2,"LEFT"}, {-5, 0, 0, 16}, "영향력 2:")
	...
	controls.maxPriceLabel = new("LabelControl", {"RIGHT",controls.maxPrice,"LEFT"}, {-5, 0, 0, 16}, "^7최대 가격:")
	...
	controls.maxLevelLabel = new("LabelControl", {"RIGHT",controls.maxLevel,"LEFT"}, {-5, 0, 0, 16}, "최대 레벨:")
	...
	if slot and not isJewelSlot and not isAbyssalJewelSlot and not slot.slotName:find("Flask") then
		...
		controls.socketsLabel = new("LabelControl", {"RIGHT",controls.sockets,"LEFT"}, {-5, 0, 0, 16}, "빈 소켓 수:")
		...
		if not slot.slotName:find("Belt") and not slot.slotName:find("Ring") and not slot.slotName:find("Amulet") then
			...
			controls.linksLabel = new("LabelControl", {"RIGHT",controls.links,"LEFT"}, {-5, 0, 0, 16}, "연결 수:")
	...
	for i, stat in ipairs(statWeights) do
		...
		if i == 1 then
			controls.sortStatLabel = new("LabelControl", {"RIGHT",lastItemAnchor,"LEFT"}, {-5, 0, 0, 16}, "^7정렬 기준 스탯:")
		elseif i == 5 then
	...
	controls.generateQuery = new("ButtonControl", { "BOTTOM", nil, "BOTTOM" }, {-45, -10, 80, 20}, "실행", function()
		main:ClosePopup()
	...
	controls.cancel = new("ButtonControl", { "BOTTOM", nil, "BOTTOM" }, {45, -10, 80, 20}, "취소", function()
		main:ClosePopup()
	end)
	main:OpenPopup(400, popupHeight, "검색 옵션", controls)
end