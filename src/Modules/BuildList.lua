...
local buildSortDropList = {
	{ label = "이름순 정렬", sortMode = "NAME" },
	{ label = "직업순 정렬", sortMode = "CLASS" },
	{ label = "최근 수정순 정렬", sortMode = "EDITED"},
	{ label = "레벨순 정렬", sortMode = "LEVEL"},
}
...
function listMode:Init(selBuildName, subPath)
	...
	self.controls.new = new("ButtonControl", {"TOP",self.anchor,"TOP"}, {-259, 0, 60, 20}, "새 빌드", function()
		main:SetMode("BUILD", false, "이름 없는 빌드")
	end)
	...
	self.controls.searchText = new("EditControl", {"TOP",self.anchor,"TOP"}, {0, 25, 640, 20}, self.filterBuildList, "검색", "%c%(%)", 100, function(buf)
